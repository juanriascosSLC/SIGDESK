  ...
  return (
    <section className="rounded-2xl border border-border/50 bg-surface-container p-5" data-testid="form-record-summary-widget">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-black text-on-surface">Resumen del registro</h3>
      ...
      <p className="mt-2 text-xs text-on-surface-variant">{completed} de {visible.length} campos completados</p>
      {missingRequired.length > 0 && (
        <p className="mt-2 text-xs text-amber-300">Faltan: {missingRequired.map((field) => field.label).join(', ')}</p>
      )}
    </section>
  );
}
