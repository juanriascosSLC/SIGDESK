 ...
 *
 * Kept from the local WIP that reconciled with this merge: the collection
 * badge (`knowledgeCollection`/`knowledgeCollectionLabel`), a navigation aid
 * derived from `readKnowledgeMetadata`, which is now the single place that
 * strips the YAML front-matter and reads its `categoria` field — this screen
 * no longer runs its own separate regex for the same job.
 */
export default function ArticleDetail() {
  ...
  const metadata = article.data ? readKnowledgeMetadata(article.data.contenido) : null;
  const collection = article.data ? knowledgeCollection(article.data.titulo, article.data.contenido) : null;
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-on-surface-variant">
              ...
              {collection && (
                <span className="rounded-full bg-primary/10 px-2 py-0.5 font-semibold text-primary">
                  {knowledgeCollectionLabel(collection)}
                </span>
              )}
            </div>
            <h1 className="mt-2 text-2xl font-bold text-on-surface">{article.data?.titulo}</h1>
            <div className="mt-6 whitespace-pre-wrap text-sm leading-7 text-on-surface-variant">{metadata?.body}</div>
          </article>
        )}
      </div>
    </div>
